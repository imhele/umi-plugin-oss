...
export const handleAcl = (rule, fileInfoArr, acl) => {
...
export default (api) => {
    ...
    api.onBuildComplete(({ err }) => {
        if (!err) {
            ...
            const { bucket, ignore: { extname = ['.html', '.htm'] }, } = options;
            ...
            (async function () {
                ...
                let fileInfoArr = readdirSync(absOutputPath).map(name => [name, path.join(absOutputPath, name), 'private']);
                ...
                api.logger.log(`The following files will be uploaded to ${bucket.endpoint || bucket.name}/${prefix}:\n${fileInfoArr
                    .map(fileInfo => {
